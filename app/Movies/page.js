...
const Movies = () => {
    ...
    const  FetchApiData = async () => {
        const resp = await fetch(`https://api.themoviedb.org/3/movie/popular`, options)
    ...
    const MovieCards = ApiData.map((d,id)=>{
        return(
            <div className="MovieCard" key={d.id} onClick={()=>{DeatilHandler('movie',d.id)}}>
    ...
    <>
        ...
        <div className="Wrapper">
            ...
            {/* <div className='PaginationWrapper'>
                <button className='leftBtn' onClick={()=>{paginationHandler('left')}}>{'<'}</button>
                    <div className='PageValue'>
                        <span>{pageIndex}</span>
                    </div>
                <button className='rightBtn' onClick={()=>{paginationHandler('right')}}>{'>'}</button>
            </div> */}
